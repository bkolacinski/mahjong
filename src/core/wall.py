...
class Wall:
    tiles: list[Tile]
    dead_wall: list[list[Tile | bool]]
    dead_wall_size: int

    def __init__(
        self,
        tiles_saved_state: (
            tuple[list[Tile], list[list[Tile | bool]]] | None
        ) = None,
    ...
    def __str__(self) -> str:
        return (
            f'wall: {" ".join(str(tile) for tile in self.tiles)}\n'
            + f'dead wall: {" ".join(str(tile) for (tile, _) in self.dead_wall)}'
        )

    def get_dora_indicators(self) -> list[Tile | bool]:
        return [tile for tile, revealed in self.dead_wall if revealed]

    def get_ura_dora_indicators(self) -> list[Tile | bool]:
        return [
            self.dead_wall[self.dead_wall_size // 2 + i][0]
...
if __name__ == "__main__":
    ...
    print(
        "Dora indicators:",
        " ".join(str(tile) for tile in wall.get_dora_indicators()),
    )
    print(
        "Ura Dora indicators:",
        " ".join(str(tile) for tile in wall.get_ura_dora_indicators()),
    )
